var  MedicineReader = function(){
  ...
  my.removed_list = []; // keep list of items that have been removed (eg for pasting)
   ...
   * If we are in remove mode, then instead try and remove the entity
  */
  my.list_click = function(entity, listname){
    if(my.is_remove_mode){ // in remove mode, clicking
      if(my.editing_list.toLowerCase()===listname.toLowerCase()){
  ...
  my.add_entity_clicked = function( listName ){
    ...
    if(my.selected_entities.length>0 && my.editing_list!=="Synonyms"){
      ...
      $(".select-entity-dialog-paste-text").html(button);
    }else{
      button.hide();
    }
  };
  ...
  my.visualise = function(){
    ...
    // probably shouldn't do these more than once @TODO
    ...
    $("#visualisation-dialog .button-ok").click(function(){
      ...
      my.internal_navigate(my.vis.current_entity);
      $("#visualisation-dialog").removeClass("show"); // close
    });
  ...
  my.vis_reload_nodes = function(){
    var graph = my.create_nodes_json(my.vis.current_entity, my.vis.max_depth);
    ...
    // connector length
    setTimeout(function(){
      my.vis.cola_adaptor.constraints(graph.constraints)
        .linkDistance(120)
        .start();
    }, 1000); 
   ...
   */
  my.create_nodes_json = function(ent_name, depth, json, layer){
    if(json===undefined){
    ...
    if(ent.hasOwnProperty("Causes")){
      ent.Causes.forEach( e => {
        ...
        json = my.create_nodes_json(e, depth-1, json, layer-1);
      });
    }
